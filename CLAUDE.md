...
- **Figure styling lives in `scripts/figures/style.py`** as a single `apply_style()` (8×6 inches at 900 dpi, Helvetica Now Micro falling back to Helvetica, combined color+marker cycle using the Okabe-Ito colorblind-safe palette). Each figure script imports it as `from style import apply_style` — the bare import works because Python prepends the script's directory to `sys.path`, same as `scripts/data/odds.py`.
- The `edu_order` / `read_order` dicts you'll see in `scripts/data/*_or.py` record first-appearance order of category values so output rows sort the same way as the stage-1 CSV.
